...
def play_mcts_vs_greedy(first_player='MCTS'):
    memo = {}
    print("MCTS = X | Greedy = O")  # Adjust the player names
    boardd = [[['-', '-', '-'], ['-', '-', '-'], ['-', '-', '-']],
              [['-', '-', '-'], ['-', '-', '-'], ['-', '-', '-']],
              [['-', '-', '-'], ['-', '-', '-'], ['-', '-', '-']]]
    curr = 'X' if first_player == 'MCTS' else 'O'

    while check_win(boardd) == 'Y':
        draw_board(boardd)

        if curr == 'X':  # MCTS's turn
            print("MCTS Algorithm is thinking", end='', flush=True)
            time.sleep(1)
            for _ in range(3):
            ...
            for _ in range(3):
                ...
                time.sleep(1)
            start_time = time.time()
            move_index = greedy(copyboard(boardd), curr)
            end_time = time.time()

        move(move_index, boardd, curr)
        print(f'Move: {move_index}')
        print(f'Time taken for the move: {end_time - start_time} seconds')
...
if __name__ == "__main__":
    ...
    user_starts = input("Do you want MCTS to go first? (y/n): ").lower() == 'y'
    play_mcts_vs_greedy('MCTS' if user_starts else 'Greedy')
